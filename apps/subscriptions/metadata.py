...
class ProductWithMetadata(object):
    ...
    def to_dict(self):
        ...
        def _serialized_price_or_none(price):
            return (
                PriceSerializer(price, context={"product_metadata": self}).data
                if price
                else None
            )
        ...
        return {
            ...
            "active_prices": {
                interval: _serialized_price_or_none(
                    self._get_price(interval, fail_hard=False)
                )
...
def get_help_text_for_interval(interval):
    return {
        PlanInterval.year: _(
            "You're getting two months free by choosing an Annual plan!"
        ),
