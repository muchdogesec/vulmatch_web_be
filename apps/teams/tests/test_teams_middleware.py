...
class TeamsAuthTest(TestCase):
    ...
    def test_team_view(self):
        ...
        response = self.client.get(
            reverse("single_team:manage_team", args=[self.sox.slug])
        )
        self.assertEqual(200, response.status_code)
        self._assertRequestHasTeam(response, self.sox, self.sox_admin, ROLE_ADMIN)
    ...
    def test_team_view_no_membership(self):
        self._login(self.sox_admin)
        response = self.client.get(
            reverse("single_team:manage_team", args=[self.yanks.slug])
        )
    ...
    def test_team_admin_view(self):
        ...
        response = self.client.post(
            reverse("single_team:resend_invitation", args=[self.sox.slug, invite.id])
        )
        self.assertEqual(200, response.status_code)
        self._assertRequestHasTeam(response, self.sox, self.sox_admin, ROLE_ADMIN)
    ...
    def test_team_admin_view_denied(self):
        ...
        invite = self._create_invitation()
        response = self.client.post(
            reverse("single_team:resend_invitation", args=[self.yanks.slug, invite.id])
        )
        self.assertEqual(404, response.status_code)
    ...
    def _create_invitation(self):
        return Invitation.objects.create(
            team=self.sox,
            email="dj@yankees.com",
            role=ROLE_MEMBER,
            invited_by=self.sox_admin,
        )
