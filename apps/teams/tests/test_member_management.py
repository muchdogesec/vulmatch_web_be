...
from django.contrib.messages import Message, ERROR
...
class TeamMemberManagementViewTest(MessagesTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.admin_membership2 = Membership.objects.get(
            user=self.admin2, team=self.team
        )
        self.normal_membership = Membership.objects.get(
            user=self.member, team=self.team
        )
        self.normal_membership2 = Membership.objects.get(
            user=self.member2, team=self.team
        )

    def _get_membership_url(self, membership):
        return reverse(
            "single_team:team_membership_details", args=[self.team.slug, membership.pk]
        )

    def _get_remove_membership_url(self, membership):
        return reverse(
            "single_team:remove_team_membership", args=[self.team.slug, membership.pk]
        )
    ...
    def test_admins_can_view_all_members(self):
        ...
        for membership in [
            ...
            self.admin_membership2,
            self.normal_membership,
        ]:
            response = c.get(self._get_membership_url(membership))
            self.assertEqual(200, response.status_code)
            self.assertTrue(
                membership.user.get_display_name() in response.content.decode("utf-8")
            )
    ...
    def test_admins_can_remove_members(self):
        ...
        self.assertFalse(
            Membership.objects.filter(pk=self.normal_membership.pk).exists()
        )
    ...
    def test_admins_can_remove_admins(self):
        ...
        # confirm member removed
        self.assertFalse(
            Membership.objects.filter(pk=self.admin_membership2.pk).exists()
        )
    ...
    def test_members_can_view_own_membership(self):
        ...
        self.assertEqual(200, response.status_code)
        self.assertTrue(
            self.member.get_display_name() in response.content.decode("utf-8")
        )
    ...
    def test_members_cant_change_own_role(self):
        for membership in [self.normal_membership, self.admin_membership]:
            ...
            if original_role == roles.ROLE_MEMBER:
                ...
                self.assertMessages(
                    response,
                    [Message(ERROR, "You aren't allowed to change your own role.")],
                )
    ...
    def test_members_cant_remove_other_members(self):
        c = Client()
        c.force_login(self.member)

        response = self._remove_member(c, self.normal_membership2)
        self.assertEqual(403, response.status_code)
        self.assertTrue(
            Membership.objects.filter(pk=self.normal_membership2.pk).exists()
        )
